...
def send_Notification():
    ...
    try:
        ...
        status = "success"
        statuswho.GENERIC_STATUS
    except:
        status = "error"
        statuswho.DB_CONNECTION_FAILED

    if status == "success":
        try:
            SQL = "select parkingid,parkingemail from " + activepark_view + \
                " where timeremaininginminutes<'10' and notificationSent='0'"
            cur.execute(SQL)
            result = cur.fetchall()
        except:
            status = "error"
            statuswho.TABLE_DOESNOT_EXIST
        if result == []:
            result = 0
            status = "error"
            statuswho.LOGIN_STATUS_FAIL
        else:
            for i in range(len(result)):
                ...
                try:
                    ...
                    SQL = "update " + activepark_view + \
                        " set notificationSent='1' where parkingemail in ('" + \
                        parkingemail+"') and parkingid in ('"+parkingid+"')"
                    cur.execute(SQL)
