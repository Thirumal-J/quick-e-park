# Input File for providing DB details, table name, common string formats #Importing Constant variables file
...
def UpdateUsertable(tableName, name, surname, email, mobileno):
    ...
    try:
        if (isTableExist(tableName)):
            status = "success"
            if not name is None:
                staticVar.cursor.execute(
                    " UPDATE "+tableName+" SET name ='"+name+"' WHERE email ='"+email+"';")
            else:
                print("name is null")
            if not surname is None:
                staticVar.cursor.execute(
                    " UPDATE "+tableName+" SET surname ='"+surname+"' WHERE email ='"+email+"';")
            else:
                print("surname is null")
            if not mobileno is None:
                staticVar.cursor.execute(
                    " UPDATE "+tableName+" SET mobileno ='"+mobileno+"' WHERE email ='"+email+"';")
            else:
                print("surname is null")
...
def showVehicleData(tablevehicle, tableuser, email):
    ...
    try:
        ...
        fetch_query = "SELECT carregistrationno,vehicletype FROM "+tablevehicle+" INNER JOIN " + \
            tableuser + " ON "+tablevehicle+".uid=" + \
            tableuser+".uid WHERE email='"+email+"' ;"
        print("*********", fetch_query)
        ...
        for row in uservehicleData:
            ...
            results.append({"carRegNumber": carRegNumber,
...
def addVehicleData(tablevehicle, tableusers, carregistrationno, vehicleType, email):
    ...
    try:
        ...
        select_query = "SELECT vehicletype FROM tbl_vehicles WHERE carregistrationno='" + \
            carregistrationno+"' AND vehicletype='owner';"
        staticVar.cursor.execute(select_query)
        staticVar.cursor.fetchall()
        count = staticVar.cursor.rowcount
        if not (count == 0):
            if (vehicleType == 'owner'):
                select1_query = "SELECT carregistrationno,vehicletype FROM tbl_vehicles INNER JOIN tbl_user ON tbl_vehicles.uid=tbl_user.uid WHERE email='" + \
                    email+"' AND carregistrationno='"+carregistrationno+"' AND vehicletype='owner';"
                staticVar.cursor.execute(select1_query)
                staticVar.cursor.fetchall()
                countown = staticVar.cursor.rowcount
                if not (countown == 0):
                    result = {
                        "registrationMsg": "You are the owner of this vehicle and it already exists in your profile."}
                    status = "error"
                    status_who = statuswho.INSERTION_CARREGNO_FAIL
                    return retcommon_status.createJSONResponse(status, status_who, result)
                else:
                    result = {
                        "registrationMsg": "Owner already exists for the vehicle"}
                    status = "error"
                    status_who = statuswho.INSERTION_CARREGNO_FAIL
                    return retcommon_status.createJSONResponse(status, status_who, result)
            else:
                select_query = "SELECT vehicletype FROM tbl_vehicles INNER JOIN tbl_user ON tbl_vehicles.uid=tbl_user.uid WHERE email='" + \
                    email+"' AND carregistrationno='"+carregistrationno+"';"
                staticVar.cursor.execute(select_query)
                staticVar.cursor.fetchall()
                counttemp = staticVar.cursor.rowcount
                if (counttemp == 0):
                    insert_query = "INSERT INTO "+tablevehicle + \
                        "(uid,carregistrationno,vehicleType,currentactive) VALUES((SELECT uid FROM " + \
                        tableusers+" WHERE email='"+email+"'),'" + \
                        carregistrationno+"','"+vehicleType+"','1') ;"
                    ...
                    result = {
                        "registrationMsg": "Vehicle already exists in your profile."}
                    status = "error"
                    status_who = statuswho.INSERTION_CARREGNO_FAIL
                    return retcommon_status.createJSONResponse(status, status_who, result)
        else:
            select_query = "SELECT vehicletype FROM tbl_vehicles INNER JOIN tbl_user ON tbl_vehicles.uid=tbl_user.uid WHERE email='" + \
                email+"' AND carregistrationno='" + \
                carregistrationno+"' AND vehicletype='temporary';"
            print("*********", select_query)
            staticVar.cursor.execute(select_query)
            staticVar.cursor.fetchall()
            counttemp = staticVar.cursor.rowcount
...
def checkOwner(tablevehicle, tableusers, carregistrationno):
    ...
    try:
        ...
        select_query = "SELECT vehicletype FROM "+tablevehicle + \
            " WHERE carregistrationno='"+carregistrationno+"' AND vehicletype='owner';"
        print("*********", select_query)
        staticVar.cursor.execute(select_query)
        staticVar.cursor.fetchall()
        count = staticVar.cursor.rowcount
        if not (count == 0):
            # if (vehicleType == 'owner'):
            select1_query = "SELECT email FROM "+tableusers+" INNER JOIN " + tablevehicle+" ON "+tableusers + \
                ".uid="+tablevehicle+".uid WHERE carregistrationno='" + \
                carregistrationno+"' AND vehicletype='owner';"
            staticVar.cursor.execute(select1_query)
            staticVar.connection.commit()
            uservehicleData = staticVar.cursor.fetchall()
            ...
            result = {
                "Msg": "Owner doesnot exists for this vehicle registration number."}
            status = "error"
